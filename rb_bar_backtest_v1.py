...
def cal_sign(data):
    ...
    for i in range(3,len(data)+1):
        t_grad = np.gradient(data['close'][i-3:i].values)
        if len(grad_list) == 0:
            grad_list.extend(t_grad)
        else:
            grad_list.append(t_grad[-1])

    data['true_grad'] = grad_list
    h1 = data[(data['grad'] <= data['true_grad'])&(data['grad']>=2)].index
    l1 =  data[(data['grad']>=data['true_grad'] )&(data['grad']<=-2)].index
    data['sign']=0
    data.loc[h1,'sign'] = 1
    data.loc[l1,'sign'] = -1
    return data
